...
% This chapters sound a bit uninspired. I think the same message could be said in fewer words.
...
% j: watch out, u use the same citation here than before
...
% j: Give 2 examples of common POS tags
...
\item \textit{Syntactic parsing:} is the process of producing a constituency structure from a sentence. In constituency structure, the words are organized into phrases, which are further organized into a phrase or a sentence. % j: What? -- Furthermore, what is a "constituency structure"?
...
% j: Make ALL figures/tables as much self-contained as possible, no matter repetition with the main text.
% j: For example, here you could write "Usual NLP pipeline.", or "Usual pipeline of an NLP system."
\end{figure}
...
% s: task of extracting information from emails
...
% j: I haven't checked the math exhaustively. Please, triple-check
...
% j: u put the figures side by side. Yet, the captions are not aligned. That looks wrong.
...
where $\mathbf{w}$ is a vector normal to the hyperplane and $\mathbf{x}$ are the set of all points that satisfy above equation and are located on the hyperplane. The projection length from the origin is denoted by $\frac{b}{|\mathbf{w}|}$. % j: u must explain what b and origin are
...
% j: Following previous convention, x_i should be written in bold / vector form
For such a classifier, if the target classes for a data point $x_i$ are given by $y_i \in \left\lbrace -1, +1 \right\rbrace$, then the constraints
...
% j: this is the first time you use "weights" -- You should have made it clearer before that vector w corresponds to a vector of weight points
...
\caption{Support vectors}\label{fig:SVM3} % j: this caption is not informative. What are the support vectors here?
\end{figure}
...
% j: What is overfitting? What is training & testing error?
